...
| カード | カードの使い方、角の形状 | 記事一覧と学習経路に外周と角丸ミディアム（12px）を適用 |

## Noema固有の判断

- ブランド名と技術図解にはDADSのキーカラーである青を使用する。
- 公開ブログはカードを多用せず、記事一覧と選択肢を比較する学習経路に限って外周付きの角丸カードを使用する。
- Level 1・2と開発・理論トラックは、色ではなく見出し、ラベル、説明文で関係を示す。
...
- 1440×1000と390×844で、Level 1・2、開発・理論トラックの順序と並列関係を確認。
