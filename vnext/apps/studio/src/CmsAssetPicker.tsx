    ...
    <div className="studio-dialog-backdrop">
      <section aria-labelledby="asset-picker-heading" aria-modal="true" className="studio-dialog studio-asset-picker" ref={dialog} role="dialog">
        <StudioSurfaceHeader
          eyebrow="Assets"
          onClose={onClose}
          title={mode === "hero" ? "記事画像を選択" : "本文へ画像を挿入"}
          titleId="asset-picker-heading"
        />
        <div className="studio-asset-picker__controls">
          <label htmlFor="asset-picker-search">画像を検索</label>
          <input className="studio-control" id="asset-picker-search" onChange={(event) => setQuery(event.target.value)} placeholder="ファイル名、説明、タグ" ref={search} type="search" value={query} />
          <input
        ...
        <div className="studio-asset-picker__body">
          ...
          {selected ? (
            <aside className="studio-asset-picker__selection">
              ...
              <textarea className="studio-control" id="asset-picker-alt" onChange={(event) => setAlt(event.target.value)} rows={3} value={alt} />
