...
Blog WorkerはD1のpublished revisionをruntimeでMarkdownからHTMLへ変換します。公開ブログ、Studio、Studio MCPは同じ記事presentation rendererを使います。Rendererでもraw HTMLをescapeし、危険なlink・image URLを無効化するため、validatorだけをsecurity境界にしません。HTMLのコード例はインラインコードまたはコードフェンスへ記述してください。独自記法は[Noema記事Markdown拡張](../docs/article-markdown.md)を参照してください。
...
- `GET /api/cms/articles/{id}/comments`: 未対応優先でrevision・本文アンカー・対応状態に紐づくレビューコメントを取得
- `POST /api/cms/articles/{id}/comments`: 記事全体、記事情報、または現在Markdownの選択範囲へのレビューコメントを追加
- `PATCH /api/cms/articles/{id}/comments/{commentId}`: 修正revisionで対応済みにするか、レビュー担当が未対応へ戻す
