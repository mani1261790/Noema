...
export class NoemaStack extends Stack {
  constructor(scope: Construct, id: string, props: NoemaStackProps) {
    ...
    api.addRoutes({
      path: "/api/assessments/notebooks/{notebookId}",
      methods: [apigwv2.HttpMethod.GET],
      integration: apiIntegration
    });

    api.addRoutes({
      path: "/api/assessments/notebooks/{notebookId}/attempts",
      methods: [apigwv2.HttpMethod.POST],
      integration: apiIntegration
    });

    api.addRoutes({
      path: "/api/assessments/chapters/{chapterId}/final",
      methods: [apigwv2.HttpMethod.GET],
      integration: apiIntegration
    });
    ...
    api.addRoutes({
      path: "/api/admin/notebooks/preview",
      methods: [apigwv2.HttpMethod.POST],
      integration: apiIntegration,
      authorizer: jwtAuthorizer
    });
