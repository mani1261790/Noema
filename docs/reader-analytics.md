...
この画面が扱うのはNoema内の行動です。ページ閲覧の実利用環境とCore Web VitalsはCloudflare Web Analytics、検索表示回数・検索クリック・検索語句はGoogle Search Consoleで別に確認します。StudioはSearch Consoleを「外部で確認」と表示し、URLプレフィックスプロパティ`https://noema-learn.uk/`の検索パフォーマンスへ直接移動できます。検索値をNoemaのAPIやD1へ取り込んだり、検索結果に表示された事実をサイト内イベントから推測したりしません。
