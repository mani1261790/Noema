...
3. 最初に表示される「記事を管理」画面で、「新しい記事を書く」を選ぶか、保存済みの記事の「編集する」を選びます。
...
通常、Studioは「記事を管理」画面から始まります。検索欄は記事が0件でも常に表示され、タイトル、スラッグ、更新者で検索し、「すべて」「公開中」「下書き・要修正」「レビュー・承認」「保管」で絞り込めます。検索条件は記事を開いて一覧へ戻っても保持されます。Studio全体の共通ヘッダーと公開サイトへのショートカットは置かず、編集画面では「記事一覧」「保存状態」「保存」だけを操作列に表示します。公開済みの記事を確認するリンクは、その記事を開いたときの設定内にだけ表示します。
...
記事の正本と履歴はD1です。GitはStudioのコード、schema、docsを管理し、記事本文の版管理には使いません。D1 Time Travelもdatabase全体の障害復旧用であり、記事単位の履歴操作には使いません。

Studioは保存ごとにimmutable revisionを残しながら、画面では同じ編集セッションの自動保存を1つの「版」にまとめます。記事を開き直した時、30分以上編集が空いた時、手動保存、競合解消、過去版の復元、レビュー・承認・公開の後に次の版を開始します。履歴には最新50版を新しい順に表示し、各版の保存回数、更新者、時刻、現在編集中・承認済み・公開中の状態を確認できます。版を開くと「自動保存の内訳」から保存時点を100件ずつ追加表示し、版の途中も比較・復元できます。

「この版をもとに編集」を選んでも、過去のrevisionや公開中のrevisionは上書きしません。確認後、選んだ本文と記事情報を未保存の状態で編集画面へ戻し、「保存」を押した時点で復元元revisionを記録した新しいrevisionを作ります。他の編集者がその間に更新した場合は通常の競合解消へ進みます。移行前の古いrevisionで公開範囲が記録されていない場合は、現在の公開範囲を維持します。
...
レビュー担当は、自分が保存した最新版を自分で承認できません。編集者、別のレビュー担当または管理者が承認します。
...
公開を止める場合は、編集者または管理者が「公開を終了して保管」を選びます。「未公開へ戻す」で保管状態を解除しても自動では再公開されません。承認済みrevisionがそのままなら編集者または管理者が再公開でき、復元後に内容を変えた場合は、改めてレビュー・承認してから公開します。
...
| 編集者 | 記事の作成・編集・保存、レビュー依頼、承認、修正依頼、公開、保管 |
| レビュー担当 | 記事の閲覧、承認、修正依頼 |

レビュー担当は、自分が保存した最新revisionを自己承認できません。編集者と管理者は自分が保存した最新版も承認し、公開できます。無効化されたメンバーや未招待のidentityは、Accessを通過してもCMS APIを利用できません。
...
Studioは競合時も入力中の内容とブラウザの復旧コピーを保持し、エディター直下に競合解決画面を表示します。そこではブラウザ原稿とCMS最新版のrevision、更新者、更新時刻を確認し、次を選べます。

- ブラウザの原稿を採用する
- CMS最新版を採用する
- ブラウザの原稿をMarkdownとして書き出す
- 記事情報は項目ごと、本文は変更ブロックごとに残す側を選んで統合する

ブラウザ原稿または統合結果を採用しても、その場ではCMSへ保存しません。最新版のlock versionへ接続した未保存原稿として編集画面へ戻し、内容を確認して「保存」を選ぶと新しいrevisionになります。その間にCMSがもう一度更新された場合は、再び競合として停止します。
...
別のメンバーが先に記事を更新しています。入力内容は保持され、エディター直下に競合解決画面が表示されます。ブラウザ原稿とCMS最新版を比較し、全文または変更箇所ごとに残す内容を選んでください。統合結果は未保存のまま編集画面へ戻るため、確認後に「保存」を選びます。自動上書きは行いません。CMS最新版を取得できない場合は、再読込するかブラウザ原稿をMarkdownとして書き出せます。

### 公開したのにブログへ出ない

次を順に確認します。

1. review状態が承認済みで、編集者または管理者が公開操作まで完了したか
