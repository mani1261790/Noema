...
Noema を、Jupyter Notebook を実行して学ぶサービスから、AIを使う最初の体験と、開発・理論の深い理解を支える独立した教材サービスへ作り直す。
...
入口ではAIの有用性をまだ見つけられていない人へ、目に見える成果を先に提供する。その後は「体験する」「活用する」を共通入口として、開発と理論の並列トラックへ接続する。詳細は[コンテンツ・学習導線仕様](./content-strategy.md)を正とする。
...
収益化を目的としない。Noemaは独立した教材ブランドと編集方針を保ち、OIFはAboutページ内の運営情報としてのみ示す。
...
| URL | 役割 |
| --- | --- |
| `/` | Noema の説明、Level 1・2、開発・理論トラック、新着記事、主要テーマ |
| `/articles` | 記事一覧。学習段階、発展トラック、テーマ、キーワードで絞り込み |
...
1. パンくずリスト
2. 学習段階、発展トラック、テーマ、読了時間
3. タイトルと要約
4. この記事でできるようになること、前提知識
5. 公開日、更新日、著者
6. 目次
7. 本文
8. 参考資料・出典
