...
2. remote D1 migrationを適用する
3. `noema-learn-production`のcodeをdeployする
4. 既存のCloudflare DNSを維持し、Route APIで`noema-learn.uk/*`を公開ゲートから本番ブログへ原子的に移す
...
GitHub-hosted runnerが`noema-learn.uk`、Studio、Studio MCPのedge policyで403になる場合、本番workflowはRoute APIでapexが本番Workerを指すことを確認したうえで、本番Workerの`workers.dev` originに対してcanonical、robots、privacy、開発版との公開記事パス一致、各記事の200応答を検証します。StudioとMCPも期待する302/401またはedge境界の403を確認します。一般回線からのapex、Studio、MCP確認は、この自動検証とは別の本番user-path確認として扱います。
